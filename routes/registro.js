...
/****************************************************************************
 * Consulta - Concesionarios Instalación de convertidores
 ****************************************************************************/
app.get('/concesionario-instalacion', verificaToken, (req, res) => {

    try {
        let etiquetaLOG = ruta + '[Usuario: ' + req.usuario.IdUsuario + '] METODO: concesionario-instalacion';
        logger.info(etiquetaLOG);
        // Del token
        let pUsuarioOperacion = req.usuario.IdUsuario;
        ...
        persona.consultaConcesionarioInstalacion(parametrosModel)
            .then(result => {
                let resultado = JSON.stringify(result);
                let datos = JSON.parse(resultado);
